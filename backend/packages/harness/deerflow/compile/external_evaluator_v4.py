"""外部 evaluator v4：隔离系统 oracle，并强制校验预注册交付集合。"""
...
from deerflow.compile import external_evaluator_v3 as v3
...
EXTERNAL_EVALUATOR_SCHEMA_VERSION = v3.EXTERNAL_EVALUATOR_SCHEMA_VERSION
EXTERNAL_EVALUATOR_LAYERS = v3.EXTERNAL_EVALUATOR_LAYERS
EXTERNAL_EVALUATOR_VERSION = "forge-external-evaluator-1.3.0"
_RULES = {
    ...
    "required_candidate_artifacts": "declared_and_delivered_exact_paths_v1",
}
EXTERNAL_EVALUATOR_RULES_SHA256 = hashlib.sha256(json.dumps(_RULES, sort_keys=True, separators=(",", ":")).encode()).hexdigest()

ExternalEvaluationResult = v3.ExternalEvaluationResult
ExternalEvaluatorBackendResult = v3.ExternalEvaluatorBackendResult
ExternalEvaluatorContractError = v3.ExternalEvaluatorContractError
ExternalEvaluatorIdentityError = v3.ExternalEvaluatorIdentityError
ExternalEvaluatorLayerResult = v3.ExternalEvaluatorLayerResult
FunctionalOracleExecution = v3.FunctionalOracleExecution
FunctionalOracleSpec = v3.FunctionalOracleSpec
...
class ForgeCompileEvaluationBackend(v3.ForgeCompileEvaluationBackend):
    def __init__(
        self,
        *,
    ...
    def _evaluate_s2(
        ...
        base = super()._evaluate_s2(candidate, node_input, session, payload)
        reasons = [] if base.status == "passed" else list(base.reason_codes)
        required = node_input.initial_observation.get("required_candidate_artifacts")
        if not isinstance(required, tuple) or not required or any(not isinstance(path, str) or not path for path in required):
            reasons.append("required_artifact_contract_invalid")
        else:
            required_paths = set(required)
            declared_paths = set(candidate.artifact_paths)
            delivery_paths = {path for path, _artifact in cls._session_artifacts(session)}
            if not required_paths.issubset(declared_paths):
                reasons.append("required_artifact_undeclared")
            if not required_paths.issubset(delivery_paths):
                reasons.append("required_artifact_missing")
        return v1._layer("S2", "passed" if not reasons else "failed", *(reasons or ["candidate_artifacts_valid"]), evidence=base.evidence)

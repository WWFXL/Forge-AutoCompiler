...
V6_IDENTITY_PATH = REPO_ROOT / "benchmarks" / "runtime-identities" / "compile-runtime-v6.json"
V5_DOCUMENT_PATH = REPO_ROOT / "docs" / "compile_runtime_v5.md"
...
def test_compile_runtime_v4_and_v5_are_frozen_and_v6_hashes_match_current_product_code() -> None:
    identity = _identity()
    v5 = json.loads(V5_IDENTITY_PATH.read_text(encoding="utf-8"))
    v6 = json.loads(V6_IDENTITY_PATH.read_text(encoding="utf-8"))
    v4_predecessor = v5["predecessor"]["commit_sha"]
    v5_predecessor = v6["predecessor"]["commit_sha"]
    ...
    for path in (IDENTITY_PATH, REPO_ROOT / "docs" / "compile_runtime_v4.md"):
        ...
        historical = subprocess.run([git, "show", f"{v4_predecessor}:{relative_path}"], cwd=REPO_ROOT, capture_output=True, check=True).stdout
    ...
    for path in (V5_IDENTITY_PATH, V5_DOCUMENT_PATH):
        relative_path = path.relative_to(REPO_ROOT).as_posix()
        historical = subprocess.run([git, "show", f"{v5_predecessor}:{relative_path}"], cwd=REPO_ROOT, capture_output=True, check=True).stdout
        assert path.read_bytes() == historical
    ...
    assert v6["identity"] == "forge-compile-runtime-v6"
    assert v6["status"] == "engineering_validation"
    assert v6["authorization"] == identity["authorization"]
    for relative_path, expected_sha256 in v6["component_sha256"].items():

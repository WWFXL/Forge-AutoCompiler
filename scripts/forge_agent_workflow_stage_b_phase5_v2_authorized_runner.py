#!/usr/bin/env python3
"""执行 Issue #297 授权的 Phase 5 v2 reachability 与六项目校准。"""
...
import forge_agent_workflow_stage_b_phase5_v2_candidate_runner as candidate_runner  # noqa: E402
...
class Phase5V2AuthorizedRunnerError(RuntimeError):
    """Phase 5 v2 授权执行的 identity、evidence、预算或 cleanup 无效。"""
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise Phase5V2AuthorizedRunnerError(f"无法读取 JSON: {path}") from exc
    if not isinstance(value, dict):
        raise Phase5V2AuthorizedRunnerError(f"JSON 根节点必须是对象: {path}")
    return value
...
def _write_once(path: Path, value: dict[str, Any]) -> None:
    ...
    try:
        ...
        raise Phase5V2AuthorizedRunnerError(f"不可覆盖已存在的 evidence: {path}") from exc
...
def _git(repo_root: Path, *arguments: str) -> str:
    ...
    if result.returncode != 0:
        raise Phase5V2AuthorizedRunnerError("无法验证授权 release Git identity")
...
def _output_dir(manifest: dict[str, Any], output_dir: Path) -> Path:
    ...
    if output_dir.resolve(strict=False) != expected:
        raise Phase5V2AuthorizedRunnerError("evidence 必须写入 Phase 5 v2 冻结授权目录")
    return output_dir
...
def require_release_identity(manifest: dict[str, Any], repo_root: Path = REPO_ROOT) -> dict[str, str]:
    ...
    if branch != execution["release_branch"] or revision != origin_main:
        raise Phase5V2AuthorizedRunnerError("真实校准要求干净 main == origin/main")
    if dirty:
        raise Phase5V2AuthorizedRunnerError("真实校准要求干净工作树")
    baseline = execution["authorization_baseline_commit"]
    if _git(repo_root, "merge-base", baseline, revision) != baseline:
        raise Phase5V2AuthorizedRunnerError("当前 release 不是 Phase 5 v2 授权基线的后代")
...
def require_network_medium(manifest: dict[str, Any]) -> str:
    ...
    if medium != execution["network_access_medium"]:
        raise Phase5V2AuthorizedRunnerError(f"必须通过 {name}={execution['network_access_medium']} 记录当前网络介质")
    return medium
...
def _run_checked(command: Sequence[str], failure: str) -> str:
    ...
    if result.returncode != 0:
        raise Phase5V2AuthorizedRunnerError(failure)
    return result.stdout.strip()
...
def require_docker_identity(manifest: dict[str, Any]) -> str:
    if not _docker_socket_is_socket():
        raise Phase5V2AuthorizedRunnerError("Docker socket 不存在或不是 socket")
    hostname = os.environ.get("HOSTNAME", "").strip()
    if not hostname:
        raise Phase5V2AuthorizedRunnerError("缺少当前执行容器 HOSTNAME")
    ...
    try:
        ...
        raise Phase5V2AuthorizedRunnerError("当前执行容器 Docker identity 无效") from exc
    if container.get("Id", "").startswith(hostname) is False or container["Config"].get("Hostname") != hostname:
        raise Phase5V2AuthorizedRunnerError("HOSTNAME 与当前 Docker 容器不一致")
    if labels.get("com.docker.compose.project") != AUTHORIZED_COMPOSE_PROJECT or labels.get("com.docker.compose.service") not in AUTHORIZED_COMPOSE_SERVICES:
        raise Phase5V2AuthorizedRunnerError("真实校准必须从授权 Compose/DooD control plane 执行")
    ...
    if socket_mount is None or socket_mount.get("RW") is not True:
        raise Phase5V2AuthorizedRunnerError("Docker socket 必须以可写方式挂载")
    if not _run_checked(["docker", "version", "--format", "{{.Server.Version}}"], "Docker daemon 不可达"):
        raise Phase5V2AuthorizedRunnerError("Docker daemon server identity 为空")
    actual = _run_checked(
    ...
    if actual != expected:
        raise Phase5V2AuthorizedRunnerError("Docker image ID 与授权修订不一致")
    return actual
...
def require_zero_managed_resources() -> None:
    ...
    if names:
        raise Phase5V2AuthorizedRunnerError(f"存在 Compile Session/replay orphan: {','.join(names)}")
    paused = _run_checked(
    ...
    if paused or images:
        raise Phase5V2AuthorizedRunnerError("存在 paused parent 或受管镜像")
...
def _provider_config_preflight(manifest: dict[str, Any]) -> None:
    ...
    if configured is None or configured.model != provider["profile"]:
        raise Phase5V2AuthorizedRunnerError("config.yaml 缺少冻结 provider model")
    ...
    if endpoint is None or str(endpoint).rstrip("/") != provider["endpoint"].rstrip("/"):
        raise Phase5V2AuthorizedRunnerError("config.yaml provider endpoint 发生漂移")
    if not os.environ.get(provider["credential_env_name"]):
        raise Phase5V2AuthorizedRunnerError("provider credential env 未注入")
...
def collect_preflight(
    ...
    *,
    ...
    if require_empty and files:
        raise Phase5V2AuthorizedRunnerError("reachability 前要求授权 evidence 目录为空")
    return {
        "ready": True,
        "manifest_sha256": protocol.candidate.canonical_sha256(manifest),
        "release_revision": release["revision"],
        "qualification_result_sha256": manifest["qualification"]["result_sha256"],
...
def _create_provider_model(manifest: dict[str, Any], *, experiment_thread_id: str | None = None) -> Any:
    ...
    if bool(getattr(model, "streaming", False)):
        raise Phase5V2AuthorizedRunnerError("冻结 provider 必须使用非 streaming 请求")
    return model


def _claim_marker(path: Path, *, document_type: str, digest: str, revision: str, task_id: str | None = None) -> None:
    payload = {
        "schema_version": "forge-agent-workflow-stage-b-phase5-v2-attempt-1.0.0",
...
def _finish_marker(path: Path, *, status: str, error_class: str | None = None) -> None:
    ...
    if marker.get("status") != "started":
        raise Phase5V2AuthorizedRunnerError("attempt marker 不处于 started")
...
def execute_reachability(
    ...
    *,
    ...
    try:
        ...
        report = {
            "schema_version": "forge-agent-workflow-stage-b-phase5-v2-reachability-1.0.0",
            "document_type": "forge_agent_workflow_stage_b_phase5_v2_reachability",
            "manifest_sha256": digest,
        ...
        if not passed:
            raise Phase5V2AuthorizedRunnerError("reachability 响应、模型 identity 或 token evidence 无效")
...
def require_passed_reachability(manifest: dict[str, Any], output_dir: Path, revision: str) -> dict[str, Any]:
    ...
    digest = protocol.candidate.canonical_sha256(manifest)
    expected_response_sha256 = hashlib.sha256(reachability["expected_response"].encode("utf-8")).hexdigest()
    tokens = report.get("recorded_tokens")
    if (
        ...
        or report.get("provider") != manifest["provider_candidate"]["provider"]
        or report.get("model") != manifest["provider_candidate"]["profile"]
        or report.get("actual_model") != manifest["provider_candidate"]["profile"]
        or report.get("endpoint") != manifest["provider_candidate"]["endpoint"]
        or report.get("request_count") != 1
        or report.get("response_sha256") != expected_response_sha256
        or type(tokens) is not int
        or not 0 <= tokens <= reachability["maximum_recorded_tokens"]
    ):
        raise Phase5V2AuthorizedRunnerError("唯一 reachability 未形成同 revision 通过终态")
    return report
...
def _task_by_id(manifest: dict[str, Any], task_id: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise Phase5V2AuthorizedRunnerError(f"未知或重复 task: {task_id}")
    return matches[0]
...
def _experiment_policy(manifest: dict[str, Any], task: dict[str, Any]) -> ExperimentPolicy:
    ...
    return ExperimentPolicy(
        benchmark_id="forge-agent-workflow-stage-b-phase5-v2-authorized-v1",
        manifest_sha256=protocol.candidate.canonical_sha256(manifest),
...
def _node_input(manifest: dict[str, Any], task: dict[str, Any], session: Any, attempt_id: str) -> AgentBuildNodeInput:
    ...
    return AgentBuildNodeInput(
        ...
        initial_observation={
            "required_candidate_artifacts": tuple(task["required_candidate_artifacts"]),
...
def _oracle_spec(task: dict[str, Any]) -> FunctionalOracleSpec:
    ...
    if oracle["kind"] == "compile_and_run":
        ...
        source_path = f"/workspace/forge-phase5-v2-oracle-{task['task_id']}{suffix}"
        executable_path = f"/workspace/forge-phase5-v2-oracle-{task['task_id']}"
    ...
    if oracle["kind"] == "service_probe":
        ...
        script = (
            f"{start} >/tmp/forge-phase5-v2-uwebsockets.stdout 2>/tmp/forge-phase5-v2-uwebsockets.stderr & pid=$!; "
    ...
    raise Phase5V2AuthorizedRunnerError(f"不支持的 oracle kind: {oracle['kind']}")
...
def _safe_cleanup(session: Any) -> tuple[Any, Any]:
    ...
    if not cleanup.succeeded or finalized.finalized_at is None:
        raise Phase5V2AuthorizedRunnerError("task cleanup 或 Session terminalization 未闭合")
...
async def execute_task(
    ...
    *,
    ...
    task_id = task["task_id"]
    execution = manifest["authorized_execution"]
    attempt_id = execution["attempt_id_template"].format(task_id=task_id)
    thread_id = f"{execution['thread_id_prefix']}-{task_id}-{digest[:12]}"
    ...
    try:
        ...
        session = prepare_compile_session_impl(
            ...
            run_id=f"phase5-v2-{task_id}-{uuid.uuid4().hex}",
            task_description=f"Phase 5 v2 Stage B calibration: {task_id}",
        )
        ...
        if clone.exit_code != 0 or session.commit_sha != task["commit_sha"]:
            raise Phase5V2AuthorizedRunnerError(f"{task_id} 无法检出冻结 commit")
        primary, _detected, _suggested = inspect_build_system_impl(session=session)
        candidate_runner.validate_runtime_probe(manifest, task, session, primary)
        ...
        if node_result.candidate_submitted:
            evaluation = run_external_evaluator_v2(
                node_input=node_input,
                node_result=node_result,
                session=session,
                manager=services.manager,
                candidate_path=_candidate_path(session, attempt_id),
                evaluation_id=execution["evaluation_id_template"].format(task_id=task_id),
...
def _completed_prefix(manifest: dict[str, Any], output_dir: Path, revision: str) -> tuple[list[dict[str, Any]], int]:
    ...
    for index, task_id in enumerate(manifest["schedule"]["order"]):
        ...
        if gap_seen or not marker_path.is_file() or not result_path.is_file():
            raise Phase5V2AuthorizedRunnerError("batch evidence 不是完整连续前缀")
        marker = _load_json(marker_path)
        result = _load_json(result_path)
        expected_attempt_id = manifest["authorized_execution"]["attempt_id_template"].format(task_id=task_id)
        ...
        if (
            ...
            or result.get("attempt_id") != expected_attempt_id
            or type(result.get("candidate_generated_observed")) is not bool
            or type(result.get("candidate_submitted")) is not bool
            or not isinstance(result.get("s0_s5"), list)
            or type(recorded_tokens) is not int
            or not 0 <= recorded_tokens <= manifest["budget_candidate"]["per_task"]["max_recorded_tokens"]
            or result.get("cleanup_succeeded") is not True
            or result.get("zero_managed_resources") is not True
        ):
            raise Phase5V2AuthorizedRunnerError(f"task evidence 未闭合: {task_id}")
        completed.append(result)
        if index != len(completed) - 1:
            raise Phase5V2AuthorizedRunnerError("batch task 顺序发生漂移")
...
def _summarize(manifest: dict[str, Any], revision: str, reachability: dict[str, Any], outcomes: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "forge-agent-workflow-stage-b-phase5-v2-report-1.0.0",
        "document_type": "forge_agent_workflow_stage_b_phase5_v2_report",
        "manifest_sha256": protocol.candidate.canonical_sha256(manifest),
        "release_revision": revision,
        ...
        "parent_phase5": manifest["parent_phase5"],
        "zero_managed_resources": True,
    ...
    *,
    ...
    if marker_path.exists():
        ...
        if marker.get("status") != "started" or marker.get("manifest_sha256") != digest or marker.get("release_revision") != revision:
            raise Phase5V2AuthorizedRunnerError("batch marker 无法恢复")
    else:
        if completed:
            raise Phase5V2AuthorizedRunnerError("缺少 create-once batch marker，拒绝导入既有 task evidence")
    ...
    try:
        for task_id in manifest["schedule"]["order"][next_index:]:
            ...
            if used >= ceiling:
                raise Phase5V2AuthorizedRunnerError("batch token ceiling 已耗尽")
            ...
            if sum(item["recorded_tokens"] for item in completed) > ceiling:
                raise Phase5V2AuthorizedRunnerError("batch recorded tokens 超过冻结上限")
            require_zero_managed_resources()
...
def load_report(manifest: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, Any]:
    ...
    if report.get("manifest_sha256") != protocol.candidate.canonical_sha256(manifest):
        raise Phase5V2AuthorizedRunnerError("Phase 5 v2 report identity 发生漂移")
    return report

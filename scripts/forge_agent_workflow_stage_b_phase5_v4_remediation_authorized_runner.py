#!/usr/bin/env python3
"""执行 Issue #305 授权的 Phase 5 v4 修复重评与 Stage C 准入判定。"""
...
_SCHEMA_RENAMES = {
    "forge-agent-workflow-stage-b-phase5-v2-attempt-1.0.0": "forge-agent-workflow-stage-b-phase5-v4-remediation-attempt-1.0.0",
    "forge-agent-workflow-stage-b-phase5-v2-reachability-1.0.0": "forge-agent-workflow-stage-b-phase5-v4-remediation-reachability-1.0.0",
    "forge-agent-workflow-stage-b-phase5-v2-task-result-1.0.0": "forge-agent-workflow-stage-b-phase5-v4-remediation-task-result-1.0.0",
    "forge-agent-workflow-stage-b-phase5-v2-report-1.0.0": "forge-agent-workflow-stage-b-phase5-v4-remediation-report-1.0.0",
}
_DOCUMENT_RENAMES = {
    "forge_agent_workflow_stage_b_phase5_v2_reachability": "forge_agent_workflow_stage_b_phase5_v4_remediation_reachability",
    "forge_agent_workflow_stage_b_phase5_v2_task_result": "forge_agent_workflow_stage_b_phase5_v4_remediation_task_result",
    "forge_agent_workflow_stage_b_phase5_v2_report": "forge_agent_workflow_stage_b_phase5_v4_remediation_report",
}


class Phase5V4RemediationAuthorizedRunnerError(RuntimeError):
    """Phase 5 v4 release、适配器、证据或 Stage C 判定无效。"""
...
def _write_once_v4(path: Path, value: dict[str, Any]) -> None:
...
def _experiment_policy_v4(manifest: dict[str, Any], task: dict[str, Any]) -> Any:
    policy = _ORIGINAL_EXPERIMENT_POLICY(manifest, task)
    return replace(
        policy,
        benchmark_id="forge-agent-workflow-stage-b-phase5-v4-remediation-authorized-v1",
    )


def _oracle_spec_v4(task: dict[str, Any]) -> Any:
    spec = _ORIGINAL_ORACLE_SPEC(task)
    return replace(
        spec,
        argv=tuple(
            item.replace("phase5-v2", "phase5-v4-remediation") for item in spec.argv
        ),
    )


def _prepare_session_v4(**kwargs: Any) -> Any:
    adapted = dict(kwargs)
    run_id = adapted.get("run_id")
    description = adapted.get("task_description")
    if isinstance(run_id, str):
        adapted["run_id"] = run_id.replace("phase5-v2-", "phase5-v4-remediation-", 1)
    if isinstance(description, str):
        adapted["task_description"] = description.replace(
            "Phase 5 v2", "Phase 5 v4 remediation evaluation", 1
        )
...
def _runtime_binding() -> Iterator[None]:
    """在单进程串行执行期间绑定 v4 identity，并始终恢复父模块。"""

    if not _RUNTIME_BINDING_LOCK.acquire(blocking=False):
        raise Phase5V4RemediationAuthorizedRunnerError(
            "Phase 5 v4 runtime binding 只允许单进程串行执行"
        )
    originals = (
        base.protocol,
    ...
    try:
        ...
        base._write_once = _write_once_v4
        base._summarize = _summarize_v4
        base._experiment_policy = _experiment_policy_v4
        base._oracle_spec = _oracle_spec_v4
        base.prepare_compile_session_impl = _prepare_session_v4
...
def _normalize_without_masking_active_error(output_dir: Path) -> None:
    ...
    try:
        ...
        active_error.add_note(
            f"Phase 5 v4 evidence ownership 规范化同时失败: {type(exc).__name__}: {exc}"
        )
...
def load_report(
    ...
    if (
        ...
        raise Phase5V4RemediationAuthorizedRunnerError(
            "Phase 5 v4 report 或嵌套 outcome identity 发生漂移"
        )
    return report
...
def _load_historical_successes(
    ...
    evidence = execution["historical_success_evidence"]
    ...
    if (
        ...
        or decision.get("manifest_sha256") != protocol.PARENT_MANIFEST_CANONICAL_SHA256
    ...
    outcomes_by_id = {
        item.get("task_id"): item for item in report_outcomes if isinstance(item, dict)
    }
...
def _load_existing_adjudication(
    ...
    if (
        ...
        != "forge-agent-workflow-stage-b-phase5-cross-run-adjudication-1.0.0"
...
def _ensure_stage_c_decision(
    ...
    decision = {
        "schema_version": "forge-agent-workflow-stage-c-decision-package-2.0.0",
...
def adjudicate_stage_c(
    ...
    *,
    ...
    parent_report, parent_results = _load_historical_successes(manifest)
    ...
    for task_id in protocol.FULL_TASK_ORDER:
        if task_id in parent_results:
            result = parent_results[task_id]
            source_identity = "phase5_v3_frozen_success"
            result_sha256 = protocol.PARENT_SUCCESS_RESULT_SHA256[task_id]
        else:
            result = remediation_outcomes.get(task_id)
            source_identity = "phase5_v4_remediation"
    ...
    adjudication = {
        "schema_version": "forge-agent-workflow-stage-b-phase5-cross-run-adjudication-1.0.0",
        ...
        "parent_manifest_sha256": protocol.PARENT_MANIFEST_CANONICAL_SHA256,
        "parent_release_revision": parent_report["release_revision"],
        "parent_report_sha256": protocol.PARENT_REPORT_FILE_SHA256,
        "parent_decision_sha256": protocol.PARENT_DECISION_FILE_SHA256,
...
def load_decision(
    ...
    if (
        value.get("schema_version")
        != "forge-agent-workflow-stage-c-decision-package-2.0.0"

#!/usr/bin/env python3
"""执行 Issue #218 授权的 R3 Make reachability 与单 pair。"""
...
import forge_opaque_provenance_r3_make_candidate_runner as make_observability  # noqa: E402
import forge_opaque_provenance_r3_make_candidate_runner as make_parity  # noqa: E402
import forge_opaque_provenance_r3_make_execution_protocol as protocol  # noqa: E402

primary = legacy.primary
v2_runner = legacy.v2_runner
v3_runner = legacy.v3_runner
DEFAULT_MANIFEST = protocol.DEFAULT_MANIFEST
DEFAULT_OUTPUT_DIR = Path("/workspace/.compile-sessions/benchmark-evidence-opaque-provenance-r3-hoextdown-v1")
ARMS = ("baseline", "treatment")


class ExecutionGateError(RuntimeError):
    """R3 Make execution identity、R0 evidence、预算或 cleanup 无效。"""
...
def _output_dir(manifest: dict[str, Any], output_dir: Path) -> Path:
    ...
    if output_dir.resolve(strict=False) != expected:
        raise ExecutionGateError("evidence 必须写入 #216 冻结的 R3 Make 目录")
    return output_dir
...
def collect_preflight(
    ...
    *,
    ...
    if require_empty and entries:
        raise ExecutionGateError("reachability 前要求 R3 Make evidence 目录为空")
    return {
        "ready": True,
...
def _policy(manifest: dict[str, Any], *, arm: str, image_id: str) -> Any:
    ...
    return primary.ExperimentPolicy(
        benchmark_id="forge-opaque-provenance-r3-make-hoextdown",
        manifest_sha256=protocol.canonical_sha256(manifest),
...
def _run_pair(
    ...
    *,
    ...
    v3_runner._claim_marker(
        marker,
        kind="forge_opaque_provenance_r3_make_pair_attempt",
        manifest_sha256=digest,
        revision=preflight["release_revision"],
    )

    capture_id = f"opaque-r3-make-{uuid.uuid4().hex[:12]}"
    ...
    parent_ledger = primary.ExperimentLedger.create(
        ...
        context={
            "scope": "opaque-provenance-r3-make-controlled-parent",
            "manifest_sha256": digest,
            "case_id": manifest["case"]["case_id"],
            "reference_case_id": manifest["case"]["reference_case_id"],
        },
    )
    gate = None
    parent_active = False
    ...
    try:
        ...
        with SqliteSaver.from_conn_string(str(output_dir / "checkpoint/messages.sqlite")) as saver:
            ...
            gate = primary.lifecycle.RealLifecycleCheckpointGate(
                ...
                owner="opaque-provenance-r3-make-execution",
            )
            gate.capture(
            ...
            with asyncio.Runner() as async_runner:
                for arm in manifest["schedule"][0]["arm_order"]:
                    ...
                    ledger = primary.ExperimentLedger.create(
                        ...
                        physical_attempt_id=primary.new_evidence_id("mechanism_attempt"),
                        context={
                            "scope": "opaque-provenance-r3-make-arm",
                            "manifest_sha256": digest,
                            "arm": arm,
                            "capture_id": capture_id,
                        },

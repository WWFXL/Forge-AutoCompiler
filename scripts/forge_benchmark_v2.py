#!/usr/bin/env python3
"""Validate the runnable Forge C/C++ pilot v2 manifest."""
...
PROTOCOL_ARTIFACT_PATHS = {
    ...
    "scripts/forge_benchmark_v2.py",
    "scripts/forge_benchmark_runner.py",
    "benchmarks/schemas/forge-cpp-benchmark-v2.schema.json",
}
...
def _validate_hash_map(
    ...
    *,
    ...
    if set(hashes) != expected_paths:
        v1._fail(path, "must contain exactly the required v2 frozen artifact hashes")
...
def _validate_manifest_impl(document: Any) -> dict[str, Any]:
    ...
    if scope.get("instrumentation_blocker") is not False:
        v1._fail(
            ...
            "must be false for the runnable v2 pilot",
        )
    ...
    if forge.get("commit_sha") != BASELINE_COMMIT:
        v1._fail("manifest.forge.commit_sha", "must bind the Issue #11 implementation baseline")
    ...
    compatibility_manifest["protocol_artifact_sha256"] = {
        ...
        "benchmarks/schemas/forge-cpp-benchmark-v1.schema.json": manifest["protocol_artifact_sha256"]["benchmarks/schemas/forge-cpp-benchmark-v2.schema.json"],
    }
    v1.validate_manifest(compatibility_manifest)
    if manifest["benchmark"]["id"] != "forge-cpp-clean-replay-pilot-v2":
        v1._fail("manifest.benchmark.id", "must identify the v2 clean-replay pilot")
...
def validate_manifest(document: Any) -> dict[str, Any]:
    """Validate a v2 manifest without reading repository files."""
    try:
...
def verify_frozen_components(manifest: dict[str, Any], repo_root: Path) -> None:
    """Verify the v2 runtime and protocol files against the current clean tree."""
    validate_manifest(manifest)
    try:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    validate_parser = subparsers.add_parser(
        "validate-manifest",
        help="validate and hash a runnable v2 benchmark manifest",
    )

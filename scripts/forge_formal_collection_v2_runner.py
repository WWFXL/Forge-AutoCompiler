#!/usr/bin/env python3
"""Candidate v2 evidence runner for the Forge C/C++ benchmark protocol.
...
The v2 batch path also keeps one asyncio event loop alive for the complete
batch so provider HTTP clients are never carried across closed loops.
"""
...
import forge_formal_collection_v2_protocol as protocol_formal_collection  # noqa: E402
    ...
    return {
        ...
        "manifest_file_sha256": _sha256_file(
            ...
            / {
                ...
                protocol_formal_collection.SCHEMA_VERSION: "cpp-formal-v2-collection.json",
...
def _attempt_message(policy: ExperimentPolicy) -> str:
    return (
        f"Compile the C/C++ repository at {policy.expected_repo_url} using exact "
        f"commit {policy.expected_commit_sha}. Follow this exact tool sequence "
        "without skipping a step: prepare_compile_session -> clone_repository -> "
        'identify_build_system -> task(subagent_type="compiler") -> '
        "finalize_session. Do not call finalize_session until the compiler "
        "subagent has submitted deterministic artifacts and clean replay has "
        "been accepted."
    )
...
def run_attempt(
    ...
    *,
    ...
    try:
        ...
        with _benchmark_memory_scope(policy):
            ...
            stream_summary = async_runner.run(stream_coro) if async_runner is not None else asyncio.run(stream_coro)

#!/usr/bin/env python3
"""Validate the runnable Forge C/C++ pilot v6 protocol."""
...
BASELINE_COMMIT = "4432325202d831d4f16980b82582dfd63c109922"
...
PROTOCOL_ARTIFACT_PATHS = {
    ...
    "scripts/forge_benchmark_v6.py",
    "scripts/forge_benchmark_runner.py",
    "benchmarks/schemas/forge-cpp-benchmark-v6.schema.json",
}
...
def _validate_hash_map(
    ...
    *,
    ...
    if set(hashes) != expected_paths:
        v1._fail(path, "must contain exactly the required v6 frozen artifact hashes")
...
def _validate_manifest_impl(document: Any) -> dict[str, Any]:
    ...
    if scope.get("instrumentation_blocker") is not False:
        v1._fail(
            ...
            "must be false for the runnable v6 pilot",
        )
    ...
    if forge.get("commit_sha") != BASELINE_COMMIT:
        v1._fail(
            ...
            "must bind the Issue #40 post-build handoff baseline",
        )
    ...
    if manifest["benchmark"]["id"] != "forge-cpp-clean-replay-pilot-v6":
        v1._fail("manifest.benchmark.id", "must identify the v6 clean-replay pilot")
...
def _compatibility_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    compatibility_manifest["protocol_artifact_sha256"] = {
        ...
        "benchmarks/schemas/forge-cpp-benchmark-v1.schema.json": manifest["protocol_artifact_sha256"]["benchmarks/schemas/forge-cpp-benchmark-v6.schema.json"],
    }
    return compatibility_manifest


def validate_manifest(document: Any) -> dict[str, Any]:
    """Validate a v6 manifest without reading repository files."""
    try:
...
def validate_run_record(document: Any) -> dict[str, Any]:
    """Validate a v6 run record with explicit build-system identity facts."""
    try:
...
def build_run_record(
    *,
    ...
    """Build a v6 record without inferring observed identity from the manifest."""
    validate_manifest(manifest)
...
def verify_frozen_components(manifest: dict[str, Any], repo_root: Path) -> None:
    """Verify the v6 runtime and protocol files against the current clean tree."""
    validate_manifest(manifest)
    try:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    validate_parser = subparsers.add_parser(
        "validate-manifest",
        help="validate and hash a runnable v6 benchmark manifest",
    )

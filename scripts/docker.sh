...
# seen by the host Docker daemon.
...
start() {
    ...
    if ! docker_available; then
        echo -e "${YELLOW}The required Docker runtime is not ready.${NC}"
        echo "Make the Docker daemon and Compose available, then rerun: make docker-start"
        exit 1
    fi

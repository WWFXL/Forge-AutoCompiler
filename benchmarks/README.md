...
the frozen 180-slot schedule. The authorized original schedule orders are
`1, 2, 73, 74, 153, 154`, covering RichLab `gpt-5.5` and DeepSeek
`deepseek-v4-flash` three times each. The maximum recorded-token budget is
980,000. Slots outside this block, including formal v3 slots 8-10, remain
unauthorized.

Generate and validate the authorized child identity with:

```bash
python scripts/forge_formal_collection_v4_authorized_protocol.py generate
python scripts/forge_formal_collection_v4_authorized_protocol.py validate-manifest \
  benchmarks/manifests/cpp-formal-v4-authorized-initial-block.json
```

After the reviewed implementation is merged, run the host gate from WSL2
`Ubuntu`, then run the non-model preflight and the single dual-provider canary
inside the `deer-flow-dev` LangGraph container. The evidence directory must be
empty and no container with a formal physical-attempt label may remain before
the canary. The canary opportunity is consumed whether it succeeds or fails.
Only a successful canary permits the six-slot batch:

```bash
./scripts/require-ubuntu-native-docker.sh

/app/backend/.venv/bin/python /repo/scripts/forge_formal_collection_v4_authorized_runner.py \
  preflight \
  --output-dir /workspace/.compile-sessions/benchmark-evidence-formal-v4-authorized-initial-block \
  --skip-endpoint-check
...
Execution is strictly serial and forbids retry, fallback, replacement, and
backfill. The token boundary is checked after the current attempt has fully
terminated and before the next ledger is created. A complete six-slot block is
eligible for the preregistered paired primary analysis; a token-limited strict
prefix remains descriptive only. The deterministic report is generated by
`scripts/forge_formal_collection_v4_authorized_report.py` after collection.

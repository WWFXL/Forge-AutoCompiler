...
The Forge commit in v2 is an implementation baseline, not a self-referential protocol commit. A clean runtime HEAD must contain `d845b735576be706f79fcf0666f66c14929a52cc` as an ancestor, while every frozen runtime component must still match that baseline. This permits later protocol-only commits without allowing runtime drift.
